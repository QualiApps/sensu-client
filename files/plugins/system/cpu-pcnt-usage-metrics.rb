...
class CpuGraphite < Sensu::Plugin::Metric::CLI::Graphite
  ...
  def run
    ...
    # total cpu used
    cpu_usage = 100 * (cpu_total_diff.to_f - cpu_sample_diff['idle']) / cpu_total_diff.to_f
    cpu_usage = sprintf('%.02f', (cpu_usage * 100) / 100.0)
    ...
    output "#{config[:scheme]}.total", cpu_usage
    ok
  end
end
